...
export default function ProgressTracker({ progress }) {
  ...
  const badges = [
    ...
    { id: 'streak_3', title: 'Habit Builder', desc: 'Maintained 3-day streak', icon: '🔥', color: 'from-amber-400 to-orange-600' },
  ];
  ...
  const renderTrendChart = () => {
    ...
    const maxVal = Math.max(...values, 100) * 1.1; // scale max slightly higher
    ...
    // Build path string
    let pathD = '';
    let areaD = `M ${getX(0)} ${paddingTop + chartHeight}`; // Start area path at bottom left
    ...
    areaD += ` L ${getX(weeklyHistory.length - 1)} ${paddingTop + chartHeight} Z`; // Close area path
        ...
        <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
          {badges.map((badge) => {
            const isUnlocked = unlockedBadges.includes(badge.id) || 
              (badge.id === 'eco_aware' && weeklyHistory?.length > 0) ||
              (badge.id === 'first_challenge' && progress?.activeChallenges?.some(c => c.completed)) ||
            ...
            return (
              ...
              >
                <div className={`w-14 h-14 rounded-2xl flex items-center justify-center text-3xl shadow-sm bg-gradient-to-br ${
                  isUnlocked ? badge.color : 'from-slate-200 to-slate-300'
                }`}>
                  {badge.icon}
                </div>
                <div>
                  ...
                  <span className={`text-[9px] font-bold uppercase tracking-wider block mt-1.5 ${
                    isUnlocked ? 'text-sage-700' : 'text-slate-400'
                  }`}>
